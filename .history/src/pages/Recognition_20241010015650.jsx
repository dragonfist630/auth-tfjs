...
const Recognition = ({ userData, userId }) => {
  ...
  const drawOval = (ctx, width, height, color) => {
    ...
    ctx.lineWidth = 10; // Set the border thickness
    ctx.strokeStyle = color; // Set the border color based on detection
    ctx.stroke(); // Draw the border
    ctx.restore();
  };
  const isFaceInOval = (faceBox, canvasWidth, canvasHeight) => {
    const ovalCenterX = canvasWidth / 2;
    const ovalCenterY = canvasHeight / 2;
    const ovalWidth = canvasWidth / 5;
    const ovalHeight = canvasHeight / 4;

    // Check if the face box is within the oval's bounds
    const faceX = faceBox.x + faceBox.width / 2; // Center of the face box
    const faceY = faceBox.y + faceBox.height / 2; // Center of the face box

    const normalizedX = (faceX - ovalCenterX) / ovalWidth;
    const normalizedY = (faceY - ovalCenterY) / ovalHeight;

    return normalizedX ** 2 + normalizedY ** 2 <= 1; // Check if inside the oval
  };
  ...
  const recognize = async (net, letter) => {
    ...
    try {
      ...
      if (detections.length > 0) {
        // ... existing code ...
        const faceBox = detections[0].detection['box']; // Get the first detected face box
        const isInsideOval = isFaceInOval(
          faceBox,
          canvasRef.current.width,
          canvasRef.current.height
        );

        // Change oval color based on face detection
        const ovalColor = isInsideOval ? 'green' : 'purple';
        drawOval(
          ctx,
          canvasRef.current.width,
          canvasRef.current.height,
      ...
      drawOval(ctx, canvasRef.current.width, canvasRef.current.height);
    } catch (error) {
