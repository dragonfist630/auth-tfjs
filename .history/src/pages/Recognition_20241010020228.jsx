...
const Recognition = ({ userData, userId }) => {
  ...
  const drawOval = (ctx, width, height) => {
    ctx.save();
    ctx.beginPath();
    ctx.ellipse(
      width / 2,
      height / 2,
      width / 5, // Adjusted width for a more egg-like shape
      height / 4, // Adjusted height for a more egg-like shape
      0,
      0,
      2 * Math.PI
    );
    ctx.lineWidth = 10; // Set the border thickness
    ctx.strokeStyle = 'purple'; // Set the border color
