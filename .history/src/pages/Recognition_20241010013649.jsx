...
const Recognition = ({ userData, userId }) => {
  ...
  const drawOval = (ctx, width, height, isFaceDetected) => {
    ctx.save();
    ctx.beginPath();
    ctx.ellipse(
      width / 2,
    ...
    ctx.strokeStyle = isFaceDetected ? 'green' : 'purple'; // Change color based on face detection
  ...
  const recognize = async (net, letter) => {
    ...
    try {
      ...
      let isFaceDetected = false; // Flag to check if a face is detected

      if (detections.length > 0) {
        isFaceDetected = true; // Set flag if any face is detected
      }

      // Draw the oval with the updated color based on face detection
